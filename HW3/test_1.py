...
class TestPositive:
    ...
    def test_add_archive(self, make_folder, clear_folder, make_files):  # a создали архив
        res_add = checkout(f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2',
                           "Everything is Ok")
    ...
    def test_check_e_extract(self, clear_folder, make_files):  #
        res = list()
        res.append(checkout(f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2',
                            "Everything is Ok"))
        res.append(checkout(f'cd {data["folder_out"]}; 7z e arx2.{data["arc_type"]} -o{data["folder_ext"]} -y',
                            "Everything is Ok"))
    ...
    def test_check_x_extract_subfolder(self, clear_folder, make_files, make_subfolder):
        ...
        res.append(checkout(f'cd {data["folder_in"]}; 7z a -t{data["arc_type"]} {data["folder_out"]}/arx2',
                            "Everything is Ok"))
        res.append(checkout(f'cd {data["folder_out"]}; 7z x arx2.{data["arc_type"]} -o{data["folder_ext"]} -y',
                            "Everything is Ok"))
        for item in make_files:
            res.append(checkout(f'ls {data["folder_ext"]}', item))
