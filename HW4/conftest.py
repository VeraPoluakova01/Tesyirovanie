...
def make_folders():
    return ssh_checkout(data['host'], data['user'], data['password'],
                        f"mkdir {data['folder_tst']} {data['folder_out']} {data['folder_ext']}", "")


@pytest.fixture()
def clear_folders():
    return ssh_checkout(data['host'], data['user'], data['password'],
                        f"rm -rf  {data['folder_tst']}/* {data['folder_out']}/* {data['folder_ext']}/*", "")
...
def make_bad_files():
    ssh_checkout(data['host'], data['user'], data['password'],
                 f"cd {data['folder_tst']}; 7z a {data['folder_out']}/arx2bad", "Everything is Ok")
